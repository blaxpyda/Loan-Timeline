...
if st.button("Calculate"):
    ...
    if response.status_code == 200:
        ...
        st.subheader(f"Total Payment (Rate 1 - {rate1}%: {result['total_payment_rate1']:.2f})")
        st.subheader(f"Total Payment (Rate 2 - {rate2}%: {result['total_payment_rate2']:.2f})")
        ...
        fig, ax = plt.subplots()
        ax.plot(df1['payment_number'], df1['remaining_balance'], label=f'Rate 1 ({rate1}%)', color='blue')
        ax.plot(df2['payment_number'], df2['remaining_balance'], label=f'Rate 2 ({rate2}%)', color='orange')
